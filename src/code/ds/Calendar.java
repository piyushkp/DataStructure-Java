...
public class Calendar {
  ...
  public void schedule(Event event) {
    ...
    if (event != null) {
      _scheEvents.add(
          new Pair<Pair<Date, Integer>, Boolean>(new Pair<Date, Integer>(event.startDate, event.id),
              true));
      _scheEvents.add(
          new Pair<Pair<Date, Integer>, Boolean>(new Pair<Date, Integer>(event.endDate, event.id),
              false));
    }
  }
  ...
  public List<ConflictedTimeWindow> findConflictedTimeWindow() {
    ...
    Collections.sort(_scheEvents, new Comparator<Pair<Pair<Date, Integer>, Boolean>>() {
      @Override
      public int compare(final Pair<Pair<Date, Integer>, Boolean> o1,
          final Pair<Pair<Date, Integer>, Boolean> o2) {
    ...
    for (int i = 1; i < _scheEvents.size(); i++) {
      if (_scheEvents.get(i).getSecond()) {
        ...
        if (ongoing.size() > 1 && !start.equals(_scheEvents.get(i + 1).getFirst().getFirst())) {
          ...
          temp.addAll(ongoing);
          ConflictedTimeWindow res = new ConflictedTimeWindow(start,
              _scheEvents.get(i + 1).getFirst().getFirst(), temp);
          output.add(res);
        ...
        if (ongoing.size() > 1 && !start.equals(_scheEvents.get(i).getFirst().getFirst())) {
          Set<Integer> temp = new HashSet<Integer>();
          temp.addAll(ongoing);
          ConflictedTimeWindow res2 = new ConflictedTimeWindow(start,
              _scheEvents.get(i).getFirst().getFirst(), temp);
          output.add(res2);
          start = _scheEvents.get(i).getFirst().getFirst();
        }
  ...
  public static void main(String[] args) {
    ...
    calendar.schedule(new Event(1, new Date(114, 0, 1, 10, 0),
        new Date(114, 0, 1, 11, 0))); // 2014-01-01 10:00 - 11:00
    calendar.schedule(new Event(2, new Date(114, 0, 1, 11, 0),
        new Date(114, 0, 1, 12, 0))); // 2014-01-01 11:00 - 12:00
    calendar.schedule(new Event(3, new Date(114, 0, 1, 12, 0),
        new Date(114, 0, 1, 13, 0))); // 2014-01-01 12:00 - 13:00

    calendar.schedule(new Event(4, new Date(114, 0, 2, 10, 0),
        new Date(114, 0, 2, 11, 0))); // 2014-01-02 10:00 - 11:00
    calendar.schedule(new Event(5, new Date(114, 0, 2, 9, 30),
        new Date(114, 0, 2, 11, 30))); // 2014-01-02 09:30 - 11:30
    calendar.schedule(new Event(6, new Date(114, 0, 2, 8, 30),
        new Date(114, 0, 2, 12, 30))); // 2014-01-02 08:30 - 12:30

    calendar.schedule(new Event(7, new Date(114, 0, 3, 10, 0),
        new Date(114, 0, 3, 11, 0))); // 2014-01-03 10:00 - 11:00
    calendar.schedule(new Event(8, new Date(114, 0, 3, 9, 30),
        new Date(114, 0, 3, 10, 30))); // 2014-01-03 09:30 - 10:30
    calendar.schedule(new Event(9, new Date(114, 0, 3, 9, 45),
        new Date(114, 0, 3, 10, 45))); // 2014-01-03 09:45 - 10:45
